...
from data_models.dataset import build_dataset, InstructDataset, PreferenceDataset, InstructDatasetSample, PreferenceDatasetSample
from utils import misc
from data_models.cleaned_documents import CleanedDocument
from data_models.dataset import DatasetType, TrainTestSplit
from data_models.prompt import GenerateDatasetSamplesPrompt, Prompt
from data_models.documents import DataCategory
from configs.settings import settings

from services.generate_datasets_service.constants import *
from services.generate_datasets_service.gen_datasets_utils import *
from services.generate_datasets_service.output_parsers import ListPydanticOutputParser
...
"""
    ...
    def generate(
        ...
        if mock:
            llm = FakeListLLM(responses=[get_mocked_response(cls.dataset_type)])
        else:
...
"""
    ...
    def post_process_datasets(
        ...
        train_test_split = create_instruct_train_test_split(
...
"""
    ...
    def post_process_datasets(
        ...
        datasets = filter_short_answers(datasets)
        datasets = filter_answer_format(datasets)
        ...
        train_test_split = create_preference_train_test_split(
            datasets, test_size=test_size, random_state=42
        )
